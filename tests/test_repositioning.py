...
def test_recommendation_reports_turnover_and_cost() -> None:
    ...
    assert recommendation.transaction_cost == pytest.approx(recommendation.turnover * 5 / 10_000)
...
def test_no_trade_when_cost_exceeds_repair() -> None:
    recommendation = recommend_long_end_trade(
        ...
        5_000,
        candidates=(
            TradeCandidate("20Y Treasury", 20, 0.045, transaction_cost_bp=10_000.0),
        ),
    )
    assert recommendation is None
...
def test_cost_guard_binds_at_the_capital_value_of_the_repair() -> None:
    ...
    candidate = TradeCandidate("20Y Treasury", 20, 0.045, transaction_cost_bp=1.0)
    ...
    over = recommend_long_end_trade(
        ...
        5_000,
        candidates=(
            TradeCandidate("20Y Treasury", 20, 0.045, transaction_cost_bp=break_even_bp * 1.01),
        ),
    )
    under = recommend_long_end_trade(
        ...
        5_000,
        candidates=(
            TradeCandidate("20Y Treasury", 20, 0.045, transaction_cost_bp=break_even_bp * 0.99),
        ),
    )
    assert over is None
...
def test_recommendation_sells_when_book_is_long_long_end() -> None:
    ...
    recommendation = recommend_long_end_trade(BalanceSheet(assets, liabilities), curve, 5_000)
    assert recommendation is not None
    assert recommendation.face_value < 0
    assert abs(recommendation.after_pv01) <= 5_000
